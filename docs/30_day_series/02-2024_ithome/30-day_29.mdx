...
今天想了一天，今天雖然是地 29 天，但明天預計是寫個 30 天心得為這次鐵人賽收個尾，所以嚴格來講今天是寫技術文章的最後一天。  
既然是最後一天，到底寫些啥好咧 ~？  
想了一天齁，後來想到之前在用 Vuetify 那些 UI framework 時，時常會看到輪播的效果，自己過去在幫朋友做網站時也用過 Bootstrap 的輪播，所以今天乾脆就來寫一下如何不靠 UI framework，手工刻一個輪播效果。

今天的 code 會跟昨天一樣有 HTML、CSS、JavaScript 三個部分，但今天 CSS 也是重要的主角之一，我們會用到我們這次鐵人賽野食常用到的 `transform`。
